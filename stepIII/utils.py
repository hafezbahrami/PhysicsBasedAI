...
class MixerDataset(Dataset):
    def __init__(self, dataset_path="./", file_name="input_data.csv",
        ...
        with open(dataset_location, newline="") as f:
            ...
            for X1, X2, X3, X4, X5, Y1, Y2, Y3, Y4, _, X_syn_1, X_syn_2, X_syn_3, X_syn_4, X_syn_5, in csv_reader:
                X = torch.tensor([float(X1), float(X2), float(X3), float(X4), float(X5),
                                  float(X_syn_1), float(X_syn_2), float(X_syn_3), float(X_syn_4), float(X_syn_5)])
                Y = torch.tensor([float(Y1), float(Y2), float(Y3), float(Y4)])
